...
def format_wind_data(df, response):
...
def format_temperature_data(df, response):
    for i in range(len(df)):
        formatted_time = pd.to_datetime(df["date"][i]).strftime("%d.%b")
...
def format_humidity_data(df, response):
    for i in range(len(df)):
        formatted_time = pd.to_datetime(df["date"][i]).strftime("%d.%b")
...
def format_snowfall_data(df, response):
    for i in range(len(df)):
        formatted_time = pd.to_datetime(df["date"][i]).strftime("%d.%b")
...
def format_rain_data(df, response):
    for i in range(len(df)):
        formatted_time = pd.to_datetime(df["date"][i]).strftime("%d.%b")
...
def format_generic_weather_data(df, response):
    for i in range(len(df)):
        formatted_time = pd.to_datetime(df["date"][i]).strftime("%d.%b")
        temperature_min = round(df["temperature_2m_min"][i], 1)  
        temperature_max = round(df["temperature_2m_max"][i], 1)  
...
def process_daily_data_today(response_from_api):
    ...
    daily_data = {"date": pd.date_range(
      start = pd.to_datetime(daily.Time(), unit = "s", utc = True),
      end = pd.to_datetime(daily.TimeEnd(), unit = "s", utc = True) - pd.Timedelta(days=2),
      freq = pd.Timedelta(seconds = daily.Interval()),
      inclusive = "left"
    )}
    daily_data["weather_code"] = daily_weather_code
...
def process_daily_data_tomorrow(response_from_api):
    ...
    daily_wind_direction_10m_dominant = daily.Variables(10).ValuesAsNumpy()[1:-1]
...
def process_daily_data_day_after_tomorrow(response_from_api):
    ...
    daily_data = {"date": pd.date_range(
      start = pd.to_datetime(daily.Time(), unit = "s", utc = True) + pd.Timedelta(days=2),
      end = pd.to_datetime(daily.TimeEnd(), unit = "s", utc = True),
      freq = pd.Timedelta(seconds = daily.Interval()),
      inclusive = "left"
    )}
...
def get_daily_data(types, response_from_api):
  ...
  response = "For weather for the next days is as follows:</p>"
  if 'wind' in types:
      response += format_wind_data(daily_dataframe, response)
  if 'temperature' in types:
      response += format_temperature_data(daily_dataframe, response)
  ...
  if 'snow' in types:
      response += format_snowfall_data(daily_dataframe, response)
  if 'rain' in types:
      response += format_rain_data(daily_dataframe, response)
  if 'weather' in types:
      response += format_generic_weather_data(daily_dataframe, response)
  return response
